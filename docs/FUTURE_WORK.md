...
### Lists deploy — schema-UID freeze discipline (CREATE2 prescription relaxed to nonce-CREATE)
`deploy/09_lists.ts` predicts the `ListResolver` / `ListEntryResolver` addresses from the deployer **nonce** (CREATE), deterministic on the pinned fork (ADR-0037). ADR-0044 §8 originally prescribed CREATE2 so the schema UIDs (which hash the resolver addresses) survive nonce drift across live networks — but **ADR-0046 §"Supersession scope" has since clarified that the shipped nonce-deterministic CREATE, combined with deploy-time consistency assertions (the deployed address matches the precomputed CREATE address, and `ListEntryResolver.LIST_SCHEMA_UID()` matches the registered LIST schema UID), is functionally equivalent under the pinned-fork discipline.** So this is **no longer a required pre-freeze action**: CREATE2 is a documented fallback *only if the pinned-fork constraint (ADR-0037) ever loosens*, not a TODO before mainnet. Supporting intuition for why nonce-CREATE is the right shipped choice — CREATE2 ties the resolver address to the **initcode**, whereas nonce-CREATE is bytecode-independent; during active devnet iteration (the resolver bytecode keeps changing — this PR alone added `WrongSchema`) a CREATE2 address would move on every edit and churn the schema UID, while nonce-CREATE on a fresh pinned fork stays stable.

**Still open here:** the `_listAttesters` on-chain attester index (`getListAttesters`/`getListAttesterCount`, added this PR) is load-bearing storage baked into `LIST_ENTRY_SCHEMA_UID` (recorded in `docs/decisions.md`, 2026-05-30) — fold it into the schema-freeze documentation pass. (CREATE2 originally raised by Gemini, Claude 4.7, Codex ×2 in PR #20 review; relaxed by ADR-0046.)
