...
const ACCOUNT = { address: ATTESTER } as const;
...
test("submitLayered cancels at a layer boundary before opening the next wallet prompt", async () => {
  ...
  await assert.rejects(
    ...
    (error: unknown) => {
      ...
      assert.equal(error.message, SUBMIT_CANCELLED);
      return true;
    },
  );
...
test("submitLayered preserves tx hash and landed UIDs when Attested logs are missing", async () => {
  ...
  await assert.rejects(
    ...
    (error: unknown) => {
      ...
      assert.equal(error.txHash, txHashes[1]);
      assert.equal(error.landed.get("DATA"), dataUID);
      assert.match(error.message, /Attested event/);
      return true;
    },
  );
  assert.equal(writes.length, 2);
...
test("submitLayered passes the selected account and chain to every layer write", async () => {
  ...
  assert.equal(writes.length, 2);
  for (const write of writes as { account: typeof ACCOUNT; chain: typeof chain }[]) {
