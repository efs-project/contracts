...
### Canonical-payload re-encode guards for PROPERTY/ANCHOR [upgrade-safe hardening]
EdgeResolver rejects non-canonical PIN/TAG payloads by exact length (32/64 bytes; PR #24) and MirrorResolver now rejects non-canonical MIRROR payloads by re-encode-and-compare (PR #24, `keccak256(a.data) == keccak256(abi.encode(transportDefinition, uri))`, both onAttest + onRevoke). The remaining dynamic-payload decode sites without a canonical guard are **PROPERTY** and **ANCHOR** in `EFSIndexer` (both carry a `string`, so re-encode-and-compare is the robust check, the `SystemAccount._requireCanonicalAnchor` pattern). Lower-priority than the edge/mirror layer (ANCHOR is non-revocable and minted via SystemAccount.bootstrap/registerAnchor which already re-encode-checks; PROPERTY is interned content, ADR-0052). Upgrade-safe (kernel logic, no schema-string change). Add before the mainnet burn if wanted.
