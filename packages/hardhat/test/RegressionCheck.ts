...
describe("Regression Checks", function () {
    ...
    it("Check Missing Feature: MimeType Indexing", async function () {
        ...
        const tx = await eas.attest({
            ...
            data: { recipient: ZeroAddress, expirationTime: NO_EXPIRATION, revocable: true, refUID: ZERO_BYTES32, data: schemaEncoder.encode(["string"], ["files"]), value: 0n }
        });
        ...
        const txFile = await eas.attest({
            schema: anchorSchemaUID,
            data: { recipient: ZeroAddress, expirationTime: NO_EXPIRATION, revocable: true, refUID: rootUID, data: schemaEncoder.encode(["string"], ["test.png"]), value: 0n }
        });
    ...
    it("Check Ghost Child in ByAttester Index", async function () {
        ...
        const tx = await eas.attest({
            schema: anchorSchemaUID,
            data: { recipient: ZeroAddress, expirationTime: NO_EXPIRATION, revocable: true, refUID: ZERO_BYTES32, data: schemaEncoder.encode(["string"], ["root"]), value: 0n }
        });
        const rootUID = getUIDFromReceipt(await tx.wait());
        ...
        const txChild = await eas.attest({
            schema: anchorSchemaUID,
            data: { recipient: ZeroAddress, expirationTime: NO_EXPIRATION, revocable: true, refUID: rootUID, data: schemaEncoder.encode(["string"], ["child"]), value: 0n }
        });
