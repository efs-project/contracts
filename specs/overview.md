...
| PROPERTY | yes | Free-floating string value, placed on a container via PIN under a PROPERTY-typed "key" anchor (ADR-0035 → ADR-0041). A *claim/value* (not a Schelling point), so revocable (ADR-0052) — revoking withdraws the value from the default view (ADR-0051). Reserved key anchor names: `contentType` (ADR-0005), `name` (ADR-0034). |
