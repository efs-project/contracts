...
describe("EFSIndexer Ghost Child Repro", function () {
    ...
    let propertySchemaUID = ZERO_BYTES32;
    let dataSchemaUID = ZERO_BYTES32;
    let blobSchemaUID = ZERO_BYTES32;
    let tagSchemaUID = ZERO_BYTES32;
    ...
    it("Should show ghost children after revocation", async function () {
        ...
        const tx = await eas.attest({
            ...
            data: { recipient: ZeroAddress, expirationTime: NO_EXPIRATION, revocable: true, refUID: ZERO_BYTES32, data: data, value: 0n }
        });
        const receipt = await tx.wait();
        const rootUID = getUIDFromReceipt(receipt);

        // 2. Create Child "file.txt"
        const childData = schemaEncoder.encode(["string"], ["file.txt"]);
        const tx2 = await eas.attest({
            schema: anchorSchemaUID,
            data: { recipient: ZeroAddress, expirationTime: NO_EXPIRATION, revocable: true, refUID: rootUID, data: childData, value: 0n }
        });
    ...
    it("Should show ghost references after revocation", async function () {
        ...
        // 1. Create Anchor
        const data = schemaEncoder.encode(["string"], ["ref_target"]);
        const tx = await eas.attest({
            schema: anchorSchemaUID,
