...
**Status:** Accepted (James ratified 2026-06-20 — see `docs/adr/0063-redirect-read-time-resolution.md`). The **rules** here are accepted; the on-chain read-time follower **implementation is DEFERRED** (see "Implementation status" below).
...
**Related:** ADR-0050 (REDIRECT schema), ADR-0055 (WHITEOUT negative terminal — reserved here), ADR-0031 (lens first-attester-wins), ADR-0062 (depth-cap precedent), specs/02 §10 (REDIRECT fields), specs/overview.md (read flow).
...
- A "hop" is one followed navigational edge — i.e. a `symlink` (the only auto-followed kind, §2). Plain path-segment descent (`resolvePath`) is **not** a hop and is bounded separately by `MAX_ANCHOR_DEPTH = 1024` (`EFSIndexer.sol:149`, ADR-0062). The two budgets are independent: a deep path with no redirects costs 0 hops; a shallow path through 17 chained symlinks exceeds `D_MAX`.
