# ADR-0031: Lenses as URL query param with first-wins fallback
...
```
web3://<router>/path/file?lenses=alice.eth,0xBob...,carol.eth
```
...
When no `?lenses=` is supplied, fall back to `?caller=` (ADR-0017), then to deployer (ADR-0016).

## Consequences

- **Viewer sovereignty**: every URL explicitly says whose content is being shown. No silent system-curated overrides.
- **Composable curation**: users can build "my lens list" and share URLs.
- **Order matters**: `?lenses=alice,bob` and `?lenses=bob,alice` may resolve differently. This is a feature (intentional precedence) but needs UX framing — users may expect "merge by newest" semantics.
- **20-lens cap** (ADR-0026) prevents unbounded URL parsing.
...
- **No on-chain lens lists**: the URL is the list. A future enhancement could let users register lens lists as Anchors with member PROPERTYs, then reference the list by UID. Not done in v1.
