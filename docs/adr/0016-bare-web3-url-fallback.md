...
When a user navigates to `web3://<router>/path/file` with no `?lenses=` parameter, the router needs to know whose content to serve. Three options:

1. Return 404 — user must always specify lenses.
2. Show "all lenses" — aggregate across every attester. No clear precedence; potentially huge result.
...
Two-step fallback when `?lenses=` is absent:
...
- "Public view" (genuinely all-lenses aggregation) is deferred. See `docs/FUTURE_WORK.md`. The current model still respects user sovereignty: nobody sees foreign content unless they explicitly opt in via `?lenses=`.
