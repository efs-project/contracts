...
> folders are visible iff at least one lens attester has an active
...
A folder is visible in a lens-scoped directory listing only if the reader's lens chain has some reason to consider it "theirs." Pre-refactor, that reason was dual-sourced:
...
- **Source B**: an explicit `TAG(definition=dataSchemaUID, refUID=folder, applies=true)` by a lens attester — used for the "create empty folder that's visible immediately" case.
...
Folder visibility is inherently per-lens (who says this is their folder?). The TAG is already the natural per-lens claim. The write-time index was a cheap shortcut that muddied the semantics.

## Decision

Folder visibility is **single-source**: a folder appears in a lens-scoped listing iff at least one lens attester has an active applies=true `TAG(definition=dataSchemaUID, refUID=folder)`.

- Folder creation still does **not** TAG itself implicitly at the kernel layer. Membership via `ANCHOR.refUID = parentUID` remains the structural record (used by non-lens-filtered views and by the `web3://` anchor walk).
- Visibility in lens listings is a separate, per-attester property expressed through TAGs.
- The upload flow (client-side) walks the ancestor chain from the immediate parent up to root exclusive and emits a visibility TAG at every ancestor that the attester hasn't already tagged. That's how intermediate folders become visible in the uploader's lens.
...
`EFSFileView._getQualifyingTaggedFolders` now reads TagResolver's `_childrenTaggedWith` and filters on active applies=true TAGs by any lens attester. No fallback to `_qualifyingFolders`.
...
- Cross-lens visibility: if Alice uploads `/a/b/file.txt`, her visibility TAG on `/a` and `/b` doesn't make those folders visible to Bob's lens — Bob has to tag them himself. Matches ADR-0031's first-attester-wins lens model.
