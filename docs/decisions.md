...
**OPEN (human decision):** ANY-mode free text is still bytes32-packed (≤31 bytes, reorder-safe). Removing that limit needs a `string` field on LIST_ENTRY — a Tier-1 schema-UID change. The `name`-PROPERTY-per-entry alternative (ADR-0044 §7) conflicts with weight-based reorder (§7: reattest orphans attached metadata), so the string field is the cleaner fix but needs explicit go-ahead. Browser-verified end-to-end: create/open/add/reorder/**delete**/editions across two contributors. See `docs/superpowers/plans/2026-05-30-lists-adr0044-correction.md`.
