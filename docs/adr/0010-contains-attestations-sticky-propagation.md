...
> source of folder visibility** in lens-scoped directory listings. It is used
...
> visibility in lens listings comes exclusively from an active (existing, not revoked)
> `TAG(definition=dataSchemaUID, refUID=folder)` by a lens attester.
>
...
When a TAG places content at a path, EFSIndexer needs to remember which ancestor folders contain content for which attesters — this powers lens-filtered directory listings ("show me only folders Alice has touched"). The natural index is `_containsAttestations[anchor][attester] = bool`.
...
- **Ancestor flags remain sticky.** A folder that once contained content for an attester appears in their parent's lens view forever, even if all content is removed.

## Consequences

- **No false negatives**: a folder with content always appears in its attester's lens view. Discoverability is preserved.
- **False positives possible**: an empty folder may appear in a parent's lens view if it ever had content. Cosmetic noise, not a correctness issue.
