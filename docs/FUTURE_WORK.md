...
per-chain merge (ADR-0061); `networkLabel`/the switcher key off chain id. **Cutover dependency:** the
devnet VPS must run `anvil --chain-id 26001993` (was 31337) + redeploy — tracked in the `devnet/` repo;
until then, selecting Devnet in a deployed build points at a node still on 31337.
