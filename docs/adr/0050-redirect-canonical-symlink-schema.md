...
- **`kind` as `uint8`** — rejected for `uint16`. Widening is free (both are one ABI word; verified byte-identical 64-byte payload), and `kind` is an open-ended relationship vocabulary, so the irreversible field takes zero-cost headroom while still reading as a bounded discriminator.
- **`kind` as `bytes32` pointing to a kind-defining Anchor (open, TAG-style vocabulary)** — rejected. It is storage-free (one word, same as `uint8`) and consistent with TAG's anchor-pointer `definition`, but it defeats the reason REDIRECT is a first-class schema: the resolver can only enforce no-loop/typing guards for kinds it *understands*, so an arbitrary anchor-defined kind is **unenforceable** — collapsing back toward the rejected property-convention model (the enforced set lives in resolver logic regardless of field type). It would also make every kind permissionlessly *followable*, an identity-rerouting footgun. The open, self-describing, anchor-defined relationship vocabulary is already served by **TAG** (`definition` = anchor); REDIRECT stays the bounded, enforced, auto-followed set. If a general open *typed-directional-edge* primitive is wanted (the EVENT/provenance gap), design it as a TAG-family schema rather than widening `kind`.
