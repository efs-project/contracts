...
- `getDirectoryPageFiltered(parent, anchorSchema, attesters, excludeTagDef, minWeight, cursor, maxItems)` — as above, but skips any item a lens has tagged `excludeTagDef` with `weight >= minWeight` (e.g. hide `nsfw`/`system`). The threshold is a caller argument; the per-item check resolves a file's DATA via its placement PIN (file labels target the DATA UID, folder labels target the ANCHOR UID) and reads the weight via `EdgeResolver.getActiveTagWeight`. A phase-1 scan budget bounds an all-excluded page. ADR-0048.
