...
### Distinct chainId for the devnet when its contracts diverge from frozen Sepolia
Today the three debug-UI environments — local hardhat, the community devnet (VPS fork), and live Sepolia — run **identical** contract addresses (the pinned-fork determinism of ADR-0037), so the single deployed app serves them with the 2-chain model: hardhat (`31337`, labeled `Local` or `Devnet` by RPC — see `utils/scaffold-eth/networkLabel.ts`) + Sepolia (`11155111`). The switcher offers both; a community tester lands on Devnet and can flip to Sepolia.

The limit is **chainId reuse**: local hardhat and the devnet are both `31337`, so they can't be two distinct switcher entries at once, and `deployedContracts.ts[31337]` can hold only one address set. That's fine while they're identical. **When the devnet starts running contracts ahead of frozen Sepolia** (testing upgrades), give the devnet its own chainId — then it becomes a distinct wagmi chain with its own `deployedContracts` block, the per-chain merge generation (ADR-0057) already supports the divergence, and `networkLabel`/the switcher pick it up for free. Until then, the RPC-based Local/Devnet split is the only distinction and no new infra is needed.
