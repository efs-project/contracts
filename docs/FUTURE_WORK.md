...
### ANCHOR field name `schemaUID` → `anchorType`/`constraintSchema`? [NEEDS JAMES — schema-UID-changing, now-or-never]
(From the 2026-06-15 field-string sweep.) The ANCHOR field string `"string name, bytes32 schemaUID"` names a field `schemaUID` that shadows EAS's own native `Attestation.schema` — every future SDK/subgraph author meets two different `bytes32` "schemaUID"s on one object. Renaming to `anchorType`/`constraintSchema`/`containedSchema` would remove the permanent collision but **changes the ANCHOR schema UID** (Tier-1, impossible after freeze). NOT done — pending James's explicit decision (yes/no + name). This is the one open freeze-surface judgment call from the reviews.
