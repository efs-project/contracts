    ...
    return (
        <div className="container mx-auto p-4 md:p-8">
            ...
            <div className="join w-full max-w-2xl mb-8">
                ...
                />
                <button className="btn btn-primary join-item" onClick={() => navigateToUid(inputUid)}>Go</button>
            </div>
...
function AttestationDetails({ uid, onNavigate, registry }: { uid: string, onNavigate: (uid: string) => void, registry: ReturnType<typeof useSchemaRegistry> }) {
    ...
    if (!attestation || attestation.uid === zeroHash) return <div className="alert alert-error">Attestation not found (returned zero UID)</div>;

    const isRoot = attestation.uid === registry.rootTopicUid;

    // Identify Schema
    const schemaName = Object.entries(registry.schemas || {}).find(([, val]) => val === attestation.schema)?.[0] || "UNKNOWN";
            ...
            <div className="card bg-base-100 shadow-xl overflow-visible">
                <div className="card-body">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                        ...
                        <DetailRow label="Recipient" value={attestation.recipient === "0x0000000000000000000000000000000000000000" ? "None" : <Address address={attestation.recipient} />} />
                        <DetailRow label="Time" value={new Date(Number(attestation.time) * 1000).toLocaleString()} />
                        <DetailRow label="Expiration" value={attestation.expirationTime === 0n ? "Never" : new Date(Number(attestation.expirationTime) * 1000).toLocaleString()} />
...
function DecodedData({ schemaUID, data, schemas }: { schemaUID: string, data: string, schemas: any }) {
    try {
        ...
        if (schemaUID === schemas.TAG) {
            ...
            let text = "";
            try { text = hexToString(val, { size: 32 }).replace(/\0/g, ''); } catch { }
            return (
        ...
        if (schemaUID === schemas.BLOB) {
            ...
            if (isText) {
                try { displayText = hexToString(content) as any; } catch { }
            }
            return (
...
function ReferencingAttestations({ uid, onNavigate, registry }: { uid: string, onNavigate: (uid: string) => void, registry: ReturnType<typeof useSchemaRegistry> }) {
    ...
    return (
        <div className="flex flex-col gap-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                <ReferencingList title="Tags" schema={registry.schemas.TAG} target={uid} onNavigate={onNavigate} registry={registry} />
                <ReferencingList title="Properties" schema={registry.schemas.PROPERTY} target={uid} onNavigate={onNavigate} registry={registry} />
                <ReferencingList title="Files" schema={registry.schemas.FILE} target={uid} onNavigate={onNavigate} registry={registry} />
                <ReferencingList title="Blobs" schema={registry.schemas.BLOB} target={uid} onNavigate={onNavigate} registry={registry} />
            </div>
        </div>
    );
}

function ReferencingList({ title, schema, target, onNavigate, registry }: { title: string, schema: string, target: string, onNavigate: (uid: string) => void, registry: ReturnType<typeof useSchemaRegistry> }) {
    ...
    return (
        <div className="card bg-base-200 shadow-sm border border-base-300">
            <div className="card-body p-4">
                <h4 className="card-title text-sm">{title} {isLoading && <span className="loading loading-spinner loading-xs"></span>}</h4>
