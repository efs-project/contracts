...
- **Lens-scoping fix shipped alongside (review finding).** `EFSFileView.getDataMirrors` returns *all* attesters' mirrors (not lens-scoped), so a foreign attester's now-arbitrary-scheme mirror could surface in a viewer's mirror list. Added `EFSFileView.getDataMirrorsByAttester` (lens-scoped, matching ADR-0013); the debug UI uses it and renders foreign mirror URIs inert. `getDataMirrors` stays as the explicit unscoped enumeration with a doc warning.
