...
- **Permissionless Writing, Curated Reading (Lenses)**: Any piece of data can be written by any user. However, reading and resolving the "state" of this data is curated via specific user contexts called **Lenses** (Address-Based Namespaces). This allows the filesystem to be viewed through a "lens" or folder state curated by an ordered list of trusted addresses.
