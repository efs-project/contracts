...
#### WHITEOUT optional hardenings (non-urgent; surfaced by the P3 review pass) [view/router-layer, redeployable]
Two consistency/defense-in-depth follow-ups from the post-WHITEOUT adversarial review. Neither is a correctness gap; both are deferred.

- **`MAX_PATH_SEGMENTS` cap on the EFSRouter URL walk** [defense-in-depth, non-urgent]. The per-segment opaque cut (ADR-0055 2-B, `EFSRouter._segmentOpaqueSuppressed`) is a ~20× constant-factor on an already-O(segments) read, and the URL segment count is *pre-existingly uncapped*. The walk is `view`-only (no gas-griefing surface on a real tx), so this is not urgent — but a `MAX_PATH_SEGMENTS` cap on the router URL walk would bound the worst case as a clean defense-in-depth measure. EFSRouter is redeployable, so this can land anytime.
- **Paged `getOpaqueDirs(attester, start, length)` reader on WhiteoutResolver** [API consistency]. Today the resolver exposes `getOpaqueDirsCount(attester)` but no enumerator, whereas the per-name side has the paged `getChildrenWhitedOut`-shaped reader. For consistency — if a client ever needs to enumerate a lens's opaque dirs — add a paged `getOpaqueDirs(attester, start, length)` co-shaped with the per-name reader. Additive view method (additive ABI on a state contract — must ship before the resolver is burned to immutable if wanted on-chain).
