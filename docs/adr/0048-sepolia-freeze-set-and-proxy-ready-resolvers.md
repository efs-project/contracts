...
> **Note (superseded by ADR-0049):** the clause "DATA stays `bytes32 contentHash, uint64 size`" no longer holds. ADR-0049 reshaped DATA to an **empty** schema (pure identity); `contentHash` and `size` are now reserved-key PROPERTYs bound to the DATA UID, not DATA fields. The freeze table and `deploy/lib/schemas.ts` carry the empty-DATA field string. The rest of §1 (the schema set, the revocable flags incl. PROPERTY `false` per ADR-0052, the drop/defer calls) stands.
