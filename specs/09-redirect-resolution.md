...
- A "hop" is one followed navigational edge — i.e. a `symlink` (the only auto-followed kind, §2). Plain path-segment descent (`resolvePath`) is **not** a hop and is bounded separately by `MAX_ANCHOR_DEPTH = 256` (`EFSIndexer.sol:149`, ADR-0065). The two budgets are independent: a deep path with no redirects costs 0 hops; a shallow path through 17 chained symlinks exceeds `D_MAX`.
