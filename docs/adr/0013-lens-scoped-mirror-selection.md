# ADR-0013: Lens-scoped mirror selection
...
`_getBestMirrorURI` only considers MIRRORs whose attester matches the **lens attester** — the address whose TAG resolved the DATA at this path.
...
- **Mirror injection attacks blocked**: third parties cannot override what the legitimate lens attester serves.
- The viewer's `?lenses=` parameter implicitly determines which set of MIRRORs is even considered.
- A lens attester can still spam their own MIRRORs — but that's self-DoS, not an external attack.
- Mirrors attested by attesters NOT in `?lenses=` are ignored entirely. If you want your mirror to be considered for a DATA, you must be one of the lenses the viewer is looking at.
