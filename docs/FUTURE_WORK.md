...
### Demo seed runs *before* `09_lists` — demo-data edits churn List addresses (markdown-for-items PR)
`deploy/08_seed_demo_tree.ts` runs before `09_lists.ts`, so the seed's transactions consume deployer nonces *before* the List resolvers are CREATE-deployed. Any change to the demo seed's tx-count therefore shifts `ListResolver` / `ListEntryResolver` / `ListReader` addresses (and thus the `LIST` / `LIST_ENTRY` schema UIDs that hash them) even on the pinned fork — `deploy-pin-check` then fails until `deployedContracts.ts` is regenerated. This PR's on-chain Overview README seeding tripped exactly this (regenerated the three List addresses). **Fix: move the demo seed to run after all contract-deploying steps (renumber `08_seed_demo_tree` to follow `09_lists`)** so demo data can never perturb contract addresses. Cheap, devnet-only, and removes a recurring pin-regen tax on every seed edit. Pairs naturally with the CREATE2 migration above (which makes the List addresses bytecode-derived and seed-independent regardless of order).
