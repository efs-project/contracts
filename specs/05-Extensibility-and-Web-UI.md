...
- **Extensible Rendering**: Because BLOBs store `contentType`, the frontend should use a modular renderer pattern. If the `contentType` is `image/png`, it loads an Image component. If it's a 3D model, it loads a WebGL viewer. If it's a recognized new MIME type, developers can inject new viewers permissionlessly.
...
- If developers wish to create a "Social Media App" on top of EFS, they do not need a new protocol. They can define a standard folder convention (e.g., `/users/0x123/posts/`) and create `Data` attestations mapping to text `BLOB`s. 
