...
### Curated-list create can reuse an *active* slot and silently supersede a live list
`CreateItemModal`'s list-create `resolveAnchor` reuse (the dead-slot recovery path) does not distinguish a *dead* slot (anchor exists, no active LIST PIN) from a *live* one (the connected user already has an active placement PIN there). Re-creating a list with the same name in the same folder when a live list already exists creates a fresh LIST + placement PIN; the PIN is cardinality-1 per `(attester, definition, targetSchema)`, so it **supersedes the existing placement and the old list disappears from the folder**. The on-chain LIST + its entries persist (re-placeable), so it is a UX footgun, not permanent data loss — but it *reads* as data loss. Fix: before reusing the slot, check `getActivePinSlot` for the connected user; if an active LIST PIN exists, open the existing list (or warn) instead of replacing it — only treat the slot as recoverable when the active PIN is missing/revoked. Ephemeral debug UI; **deferred per maintainer de-scoping of debug-UI polish (2026-05-31)**. Codex PR #20 (3331167323).
