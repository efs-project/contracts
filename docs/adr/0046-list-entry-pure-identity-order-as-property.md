...
- **Schema-UID change.** New LIST_ENTRY schema UID + `ListEntryResolver` redeploy (CREATE2, deterministic). Free on devnet, before any real data; would be impossible post-mainnet-freeze — which is why it lands now.
