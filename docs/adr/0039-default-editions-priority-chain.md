# ADR-0039: Default editions priority chain
...
ADR-0031 established `?editions=<csv>` as the URL-encoded editions list and nailed down two properties:

- **First-attester-wins** inside the list.
- **Wholesale override** when the param is present — `?editions=alice,bob` means exactly `[alice, bob]`, nothing appended, nothing prepended. Same URL → same view for every viewer; this is what makes edition-scoped links shareable.
...
When `?editions=` is **absent**, the default editions chain is:

```
connectedAddress
  → viewedAddress (address container only)
  → webOfTrust[]     (user-configured; empty today)
  → systemEditions[] (tail fallback)
```

When `?editions=` is **present**, it overrides wholesale. Unchanged from ADR-0031 and ADR-0033.
...
- **systemEditions[]** — tail fallback. On devnet: `[DEVNET_BOOTSTRAP_CURATOR, deployerAddress]`. The deployer is read at runtime from `EFSIndexer.DEPLOYER` (immutable set in constructor). The bootstrap curator is a module-level constant in `utils/efs/containers.ts`:
  ```
...
- URL shareability intact. `?editions=alice,bob` means exactly `[alice, bob]` to every viewer, everywhere.
...
- Anyone reading the URL bar can no longer reason about "who's attesting the content I see" without knowing the default chain. The Editions chip in the toolbar should eventually surface the effective list; out of scope here.
...
### A. Merge `?editions=` into the chain at position 3 (rejected)
...
### B. Keep `?editions=` wholesale; extend only the default chain (adopted)
...
### C. Hybrid — append system tier as tail fallbacks even when `?editions=` is present (rejected)
`?editions=alice,bob` would become `[alice, bob, <system tier…>]`. Softer than A — your own wallet isn't injected — but still breaks the "URL means exactly what it says" invariant. A link saying "look at Alice's and Bob's edition of this page" must mean exactly that; appending project-blessed curators invisibly is paternalistic and undermines the core sharing property ADR-0031 is built on.
...
- `packages/nextjs/utils/efs/containers.ts` — `defaultEditionsForContainer` gains `webOfTrust` and `systemEditions` params (both optional, default empty). Export `DEVNET_BOOTSTRAP_CURATOR` constant.
- `packages/nextjs/app/explorer/[[...path]]/ExplorerClient.tsx` — reads `Indexer.DEPLOYER`, builds `systemEditions = [DEVNET_BOOTSTRAP_CURATOR, deployer]`, passes through to the hook. `webOfTrust: []` today.
- No contract changes. Defaults live entirely client-side; the router's `?caller=` fallback (ADR-0016) remains the on-chain default when no editions are resolved.
