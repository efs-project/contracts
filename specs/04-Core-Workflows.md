...
- **Step 4**: Build `multiAttest` batch:
  ...
  5. One visibility TAG per queued ancestor from Step 3: `TAG(definition=DATA_SCHEMA_UID, refUID=ancestorFolder, weight=1)`. Makes the folder appear in the attester's edition listing.
...
- **Filter per DATA**: For each DATA UID surfaced by the directory listing, call `edgeResolver.hasActiveEdgeFromAny(dataUID, funnyDefUID, [alice, bob])`. Returns true only if one of the listed attesters has an **active** (not revoked) edge on that DATA — checks both PIN and TAG schemas.
