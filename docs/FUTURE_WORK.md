...
### Non-lens directory path is unfiltered (system/nsfw leak risk)
The on-chain tag-exclusion filter (`EFSFileView.getDirectoryPageFiltered`, ADR-0048) is wired ONLY to the lens-scoped directory call. The non-lens `EFSFileView.getDirectoryPage` path has NO exclusion. Today this is harmless because the default view is always lens-scoped — `defaultLensesForContainer` always appends `systemLenses`, which always carries the devnet constants, so `lensAddresses` is never empty and the listing always takes the filtered lens path. But this is load-bearing: fold the fix into the mainnet user-configurable-lenses work (the Web-of-trust UX item under Architecture & Extensibility). When default lenses can be empty, the standard `getDirectoryPage` listing needs on-chain/client exclusion or `system` files become visible by default. See the AGENT-NOTEs at `defaultLensesForContainer` (utils/efs/containers.ts) and `systemLenses` (ExplorerClient.tsx).
