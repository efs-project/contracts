...
describe("EFSFileView", function () {
    ...
    it("Should fetch a directory page with correct metadata", async function () {
        ...
        const folderData = schemaEncoder.encode(["string"], ["Docs"]);
        const txFolder = await eas.attest({
            schema: anchorSchemaUID,
            data: { recipient: ZeroAddress, expirationTime: NO_EXPIRATION, revocable: false, refUID: rootUID, data: folderData, value: 0n }
        });
        ...
        const subData = schemaEncoder.encode(["string"], ["sub"]);
        await eas.attest({
            schema: anchorSchemaUID,
            data: { recipient: ZeroAddress, expirationTime: NO_EXPIRATION, revocable: false, refUID: folderUID, data: subData, value: 0n }
        });

        // 3. Create "File" (Anchor with Data)
        const fileData = schemaEncoder.encode(["string"], ["notes.txt"]);
        const txFile = await eas.attest({
            schema: anchorSchemaUID,
            data: { recipient: ZeroAddress, expirationTime: NO_EXPIRATION, revocable: false, refUID: rootUID, data: fileData, value: 0n }
        });
        ...
        await eas.attest({
            ...
            data: { recipient: ZeroAddress, expirationTime: NO_EXPIRATION, revocable: false, refUID: fileUID, data: contentData, value: 0n }
        });
        ...
        expect(item1.isFolder).to.be.false;
        expect(item1.hasData).to.be.true;

        expect(item2.name).to.equal("Docs");
        expect(item2.isFolder).to.be.true;
        expect(item2.hasData).to.be.false;
    });
});
