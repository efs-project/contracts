...
### Distinct chainId for the devnet — DONE (ADR-0062, chain id 5318008)
**Resolved 2026-06-22.** The devnet now has its own chain id `5318008` (ADR-0062), so Local (`31337`),
Devnet (`5318008`), and Sepolia (`11155111`) are three distinct, explicitly-selectable wagmi chains —
no more `31337` reuse / RPC-sniffing / auto-flipping. `deployedContracts[5318008]` is generated by the
per-chain merge (ADR-0061); `networkLabel`/the switcher key off chain id. **Cutover dependency:** the
devnet VPS must run `anvil --chain-id 5318008` (was 31337) + redeploy — tracked in the `devnet/` repo;
