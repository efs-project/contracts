...
- **Resolution**: Resolve the `Data` attestations to their underlying `BLOB` attestations via `blobUID`.
- **Filter**: Check the `mimeType` field of the BLOBs to match standard video MimeTypes (e.g., `video/mp4`).
...
- **Filter 2**: Resolve the Data to BLOBs and match `mimeType` with image MimeTypes (e.g., `image/jpeg`, `image/png`).
...
- **Rename (`/memes/vitalik.jpg` to `/memes/vitalikoriginal.jpg`)**: Create a new destination Anchor for `vitalikoriginal.jpg` pointing to the original BLOB. Then, issue a deletion/obfuscation attestation for the old Anchor.
